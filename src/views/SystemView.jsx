...
import { SystemIATab } from './system/SystemIATab'
...
const RenderChanges = ({ oldVal, newVal }) => {
    ...
    return (
        <div className="flex flex-col gap-1">
            {changes.map((c) => (
                <div key={c.key || `change-${c.from}-${c.to}`} className="flex items-center gap-2 text-[10px]">
...
const formatQueryContext = (context) => {
    ...
    return (
        <div className="flex flex-col gap-1 mt-1">
            {functions.map((fn) => (
                <div key={fn} className="flex items-center gap-1.5 text-[9px] font-bold text-slate-400 bg-white/5 px-2 py-0.5 rounded border border-white/5">
